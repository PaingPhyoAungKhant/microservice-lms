...
func TestGetZoomRecording_Integration_Success(t *testing.T) {
	...
	getUC := usecases.NewGetZoomRecordingUseCase(recordingRepo)

	result, err := getUC.Execute(ctx, recording.ID)
...
func TestGetZoomRecording_Integration_NotFound(t *testing.T) {
	db, cleanup := SetupTestDB(t)
	defer cleanup()
